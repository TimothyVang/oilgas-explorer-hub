 ...
 *
 * Tests invite-only investor login UI flows.
...
test.describe("Authentication - Invite-only Restrictions", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto("/login");
  });
...
test.describe("Authentication - OAuth Restrictions", () => {
  test("google sign in button is hidden", async ({ page }) => {
    await page.goto("/login");
...
test.describe("Authentication - Mobile Responsiveness", () => {
  ...
  test("invite-only login remains restricted on mobile", async ({ page }) => {
    await page.setViewportSize({ width: 375, height: 667 });
    await page.goto("/login");
...
test.describe("Authentication - Edge Cases", () => {
  ...
  test("preserves login form without signup mode", async ({ page }) => {
    await page.goto("/login");
